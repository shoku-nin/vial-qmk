...
report_mouse_t pointing_device_task_combined_user(report_mouse_t left_report, report_mouse_t right_report) {
  ...
  // Calculate cursor values based on mouse movement and divisors
    left_report.x  = left_report.x * spd_multi * CURSOR_AMP_L *(-1);
    left_report.y  = left_report.y * spd_multi * CURSOR_AMP_L *(-1);
    right_report.x = right_report.x * spd_multi * CURSOR_AMP_R;
    right_report.y = right_report.y * spd_multi * CURSOR_AMP_R *(-1);
