...
LT(1,JP_Z),MT(MOD_LALT,JP_X),JP_C,MT(MOD_LSFT,JP_V),JP_B,
  JP_N,JP_M,MT(MOD_LSFT,JP_MINS),MT(MOD_LCTL,JP_COMM),LT(2,JP_DOT),
...
[4] = LAYOUT(
TO(0),KC_NO,RGB_SPI,RGB_VAI,RGB_MOD,    RGB_MOD, RGB_VAI,RGB_SPI,KC_NO,KC_NO,
KC_NO,KC_NO,RGB_SPD,RGB_VAD,RGB_RMOD,   RGB_RMOD,RGB_VAD,RGB_SPD,KC_NO,KC_NO,
KC_NO,KC_NO,KC_NO,  KC_LSFT,KC_NO,      RGB_TOG, KC_NO,  KC_LSFT,KC_NO,KC_NO,
...
KC_NO,KC_NO,SPD_HI,KC_BTN2,KC_BTN1,    KC_BTN1,KC_BTN2,SPD_HI,KC_NO,KC_NO,
            KC_NO, SPD_HI, KC_NO,      KC_NO,  SPD_HI,KC_NO
),
...
const rgblight_segment_t PROGMEM my_layer5_layer[] = RGBLIGHT_LAYER_SEGMENTS(
    {0, 8, HSV_OFF},    {8, 1, HSV_BLUE},
    {9, 2, HSV_RED},    {11, 1, HSV_BLUE},
    {12, 1, HSV_GREEN}, {13, 3, HSV_OFF},
    {16, 1, HSV_GREEN}, {17, 1, HSV_OFF},
//
    {18, 8, HSV_OFF},   {26, 1, HSV_BLUE},
    {27, 2, HSV_RED},   {29, 1, HSV_BLUE},
    {30, 1, HSV_GREEN}, {31, 3, HSV_OFF},
...
void post_process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (layer) {
        case 4:
            rgblight_blink_layer(4, 100);
            rgblight_unblink_all_but_layer(4);
            break;
    }
}
*/
...
#define SCROLL_DIV_H    12.0
#define SCROLL_DIV_V    12.0
...
report_mouse_t pointing_device_task_combined_user(report_mouse_t left_report, report_mouse_t right_report) {
  ...
  // Calculate and accumulate scroll values based on mouse movement and divisors
    scroll_accumulated_l_h += (float)left_report.x / SCROLL_DIV_H;
    scroll_accumulated_l_v += (float)left_report.y / SCROLL_DIV_V;
    scroll_accumulated_r_h += (float)right_report.x / SCROLL_DIV_H;
    scroll_accumulated_r_v += (float)right_report.y / SCROLL_DIV_V;
  ...
  // Assign integer parts of accumulated cursor values to the mouse report
    left_report.x = (int8_t)cursor_accumulated_l_x;
    left_report.y = (int8_t)cursor_accumulated_l_y;
  ...
  // Calculate and accumulate scroll values based on mouse movement and divisors
    cursor_accumulated_l_x += (float)left_report.x / CURSOR_DIV_L *(-1);
    cursor_accumulated_l_y += (float)left_report.y / CURSOR_DIV_L;
    cursor_accumulated_r_x += (float)right_report.x / CURSOR_DIV_R;
    cursor_accumulated_r_y += (float)right_report.y / CURSOR_DIV_R;
